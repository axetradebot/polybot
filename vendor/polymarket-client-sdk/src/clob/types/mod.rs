...
sol! {
    /// V2 Polymarket Exchange Order (April 2026 migration).
    ///
    /// Why: V2 removed the `taker`, `expiration`, `nonce`, and `feeRateBps` fields. Order
    /// uniqueness now comes from `timestamp` (ms since epoch). Fees are computed at match time
    /// and never embedded in the signed order. `metadata` and `builder` are bytes32 hooks and
    /// stay zero unless we join the builder program.
    #[non_exhaustive]
    #[serde_as]
...
/// V2 wire format for `POST /order` body — `taker`, `expiration`, `nonce`, `feeRateBps`
/// are gone; `timestamp` (ms), `metadata`, `builder` are added. `metadata` and `builder`
/// serialize to `0x` + 64 hex chars, which is what V2 expects for the bytes32 hex form.
#[serde_as]
#[derive(Serialize)]
