...
/// V2 timestamp field — milliseconds since UNIX epoch. Replaces the V1 `nonce` for
...
impl<K: AuthKind> OrderBuilder<Limit, K> {
    ...
    pub async fn build(self) -> Result<SignableOrder> {
        ...
        // V2: fees are computed at match time and never embedded in the signed order.
        ...
        // V2: nonce, expiration, taker fields no longer exist on the signed order.
        // The builder accessors are kept for source-compat but their values are ignored here.
        let _ = (self.nonce, self.expiration, self.taker);
...
impl<K: AuthKind> OrderBuilder<Market, K> {
    ...
    pub async fn build(self) -> Result<SignableOrder> {
        ...
        // V2: nonce, taker no longer exist on the signed order. Builder accessors retained
        // for source-compat but values are ignored.
        let _ = (self.nonce, self.taker);
